...
| **Route Resolver** | [routing.py](../../../src/cowork_agent/features/email_action_plan/routing.py) `resolve_route` / `resolve_candidate_route` | Deterministic ladder to `NO_ACTION` \| `DIRECT_PLAN` \| `RETRIEVE_RAG`. The LLM classifier proposes; the resolver owns the final route. |
