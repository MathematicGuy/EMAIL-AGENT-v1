...
| **Chat Configuration Boundary** | [`config.py`](../../../src/cowork_agent/config.py), [`app.py`](../../../src/cowork_agent/app.py) | `app.py` loads the runtime environment before chat composition. Chat settings and the optional Gemini last-resort parser only validate the resulting mapping or `os.environ`; they perform no dotenv I/O ([ADR-017](../../../tasks/adr/ADR-017-settings-parsing-is-pure.md)). |
