...
| **Dual Persistence Engine** | `POSTGRES_MODE=off` → SQLite (`mail_todo.db` / `runs.db` / `tasks.db` / `chat.db`) plus an in-process working buffer; with `DATABASE_URL` → Supabase Postgres (migrations 001–013) | **Live / Implemented** | Fully Aligned on dual-mode switch | [repositories](../../../src/cowork_agent/persistence/repositories) |
...
| **Persistence Flexibility** | Production Postgres; Redis or in-process short-term | SQLite persists local chat sessions/history/profile/task episodes and project-document metadata/chunks; short-term remains in-process (`redis_client` / `run_queue` unbound). The local worker stores document sources and Turbovec indexes on disk while Gemini provides embeddings. | **Mostly Aligned** — Redis unused; local and cloud data are separate |
| **Architecture Documentation** | Level 1 docs reflect the running system | Streams 01–03 and dashboard audited 2026-08-17; Document Ingestion Pipeline (06) audited 2026-08-20. Docs 04–05 remain to be re-audited. | **Streams 01–03, 06 + dashboard current; 04–05 backlog** |
...
7. **[07-docx-document-viewing-and-editing.md](07-docx-document-viewing-and-editing.md):** DOCX Document Viewing Subsystem, high-fidelity in-browser Word viewer (DocxViewer), A4 page-by-page layout, zoom/fullscreen controls, and extracted markdown preview. Audited 2026-08-21.

> [!NOTE]
> Streams 01–03, 06, 07 and this dashboard were audited against live source. Documents 04–05 remain useful Level 1 references and will be updated in subsequent sync passes.
