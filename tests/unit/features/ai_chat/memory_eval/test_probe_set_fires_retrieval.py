...
def test_the_probe_set_is_found() -> None:
    for probe_set_path in PROBE_SET_PATHS:
        assert _load(probe_set_path)["probes"], f"no probes found in {probe_set_path}"


def test_cue_gated_probes_actually_fire_their_retrieval() -> None:
    for probe_set_path in PROBE_SET_PATHS:
        for scope in ("episodic", "semantic"):
...
def test_invented_any_phrases_exist_somewhere_in_the_seed() -> None:
    for probe_set_path in PROBE_SET_PATHS:
        data = _load(probe_set_path)
        material = _seed_material(data)
